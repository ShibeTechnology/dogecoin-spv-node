...
const { KOINU, MIN_FEE } = require('../../src/constants')
...
test.serial('should generate a new address', async t => {
  ...
  const address = await wallet.generateNewAddress()
...
test.serial('should create a newly signed transaction', async t => {
  ...
  const amount = 10n
  const tx = await wallet.send(amount, 'nVttKKCcjJT5k3TZKe54HedpZugbwfsp5W', MIN_FEE * KOINU)
